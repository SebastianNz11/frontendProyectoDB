...
export const FormularioEnfermeros = ({
  handleInsert,
  editar,
  setEditar,
  datos,
  ...
  const onSubmit = async (data) => {
    ...
    try {
      if (editar) {
        await updateEnfermero(data); // Llama a la función de actualización
        setEditar(false); // Sale del modo de edición después de actualizar
      } else {
        handleInsert(data); // Si no está en modo de edición, inserta un nuevo enfermero
      }
  ...
  return (
    <div className="container-fluid justify-content-center pt-5">
      <form onSubmit={handleSubmit(onSubmit)}>
        <h3 className="mb-3">Ingresar Enfermero</h3>
        <input
          type="text"
